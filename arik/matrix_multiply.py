...
def get_args(argv,index):
    ...
    lowerLimit = -100
    upperLimit = 100
    n_arry=[30,10,200,300,500]
    m_arry=[30,10,200,300,500]
    n = n_arry[index]
    m = m_arry[index]
...
def main(argv):
    ...
    for index in range(5):
        ...
        numProcessors, lowerLimit, upperLimit, n, m = get_args(argv,index)
        ...
        print("\n" * 3, "Input Matrices generated with dim: n=",n,"m=",m)
        print("First Matrix", matIn1)
        print("\n" * 2)
        print("Second Matrix", matIn2)
        print("\n" * 2)
        # start_time = time.time()
        # m = np.matmul(matIn1, matIn2)
        # print('--------------------np.matmul -------------------------------')
        # print(m)
        # timeForExecSeq = (time.time() - start_time) * 1000
        #
        # print("Time taken for builtin numpy function (np.matmul) multiplication:", timeForExecSeq, "ms")

        # print('builtin dot matrix  run took {} ',(time.time() - start_time)*1000,' ms')
        ...
        print(matOutPar)
        print("\n" * 2, "Hence, with {0} cores, parallel algorithm runs faster by {1}".format(numProcessors, float(
